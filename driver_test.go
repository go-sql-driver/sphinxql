...
// See https://github.com/Go-SQL-Driver/SphinxQL/wiki/Testing
func getEnv() bool {
	once.Do(func() {
		user := os.Getenv("SPHINXQL_TEST_USER")
		if user == "" {
			user = "root"
		}

		pass := os.Getenv("SPHINXQL_TEST_PASS")

		prot := os.Getenv("SPHINXQL_TEST_PROT")
		if prot == "" {
			prot = "tcp"
		}

		addr := os.Getenv("SPHINXQL_TEST_ADDR")
		if addr == "" {
			addr = "localhost:3306"
		}

		dbname := os.Getenv("SPHINXQL_TEST_DBNAME")
		if dbname == "" {
			dbname = "gotest"
		}
...
func mustExec(t *testing.T, db *sql.DB, query string) (res sql.Result) {
...
func mustQuery(t *testing.T, db *sql.DB, query string) (rows *sql.Rows) {
...
func TestCRUD(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestCRUD", netAddr)
		return
	}

	db, err := sql.Open("sphinxql", dsn)
	if err != nil {
	...
	// Update
	res = mustExec(t, db, "UPDATE test SET value = false WHERE value = true")
	count, err = res.RowsAffected()
	if err != nil {
		t.Fatalf("res.RowsAffected() returned error: %v", err)
	}
...
func TestInt(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestInt", netAddr)
		return
	}
	...
	for _, v := range types {
		...
		mustExec(t, db, fmt.Sprintf("INSERT INTO test VALUES (%d)", in))
...
func TestFloat(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestFloat", netAddr)
		return
	}
	...
	for _, v := range types {
		...
		mustExec(t, db, fmt.Sprintf("INSERT INTO test VALUES (%f)", in))
...
func TestString(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestString", netAddr)
		return
	}
	...
	for _, v := range types {
		...
		mustExec(t, db, fmt.Sprintf("INSERT INTO test VALUES ('%s')", in))
...
func TestNULL(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestNULL", netAddr)
		return
	}
...
func TestRowsClose(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestRowsClose", netAddr)
		return
	}
...
func TestCloseStmtBeforeRows(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestCloseStmtBeforeRows", netAddr)
		return
	}
...
func TestStmtMultiRows(t *testing.T) {
	if !getEnv() {
		t.Logf("SphinxQL-Server not running on %s. Skipping TestStmtMultiRows", netAddr)
		return
	}
